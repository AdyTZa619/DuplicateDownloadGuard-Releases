...
func (a *App) startMegaPreviewForUIV854(item RemoteItem, forceFallback bool) (string, string, time.Duration, error) {
	...
	if !forceFallback {
		if streamURL, mode, ok := a.tryMegaPreviewUICacheV854(item); ok {
			return a.browserReadyMegaPreviewV8523(streamURL, mode, started)
		}
		streamURL, err := a.startMegaPreviewResumeCoalescedV856(item)
		if err == nil {
			return a.browserReadyMegaPreviewV8523(streamURL, "MEGA DIRECT RESUME", started)
		}
	...
	return a.browserReadyMegaPreviewV8523(streamURL, "MEGA TRUE FALLBACK", started)
}
