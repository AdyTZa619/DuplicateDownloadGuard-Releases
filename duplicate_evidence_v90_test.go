...
func TestDetectorNeverTurnsPerceptualScoreIntoExactV90(t *testing.T) {
	d := decorateGuardDecision(DownloadGuardDecision{Verdict: guardDuplicate, Similarity: 100, Method: "media-same-content"})
	if d.Detector.Classification == "IDENTIC" || d.Detector.Score == nil || *d.Detector.Score != 99 || d.Exact {
	...
	if d.Detector.Classification != "IDENTIC" || *d.Detector.Score != 100 {
		t.Fatal("exact evidence lost")
	}
	d = decorateGuardDecision(DownloadGuardDecision{Verdict: guardReview, Method: "media-unverified"})
	if d.Detector.Score != nil || d.Detector.Classification != "DE VERIFICAT" {
...
func TestIncompleteDetectorPreservesMeasuredScoreV90(t *testing.T) {
	...
	if d.Detector.Classification != "DE VERIFICAT" || d.Detector.Score == nil || *d.Detector.Score != 72 || d.Detector.Pending != 10 {
	...
	if d.Detector.Classification != "DE VERIFICAT" {
