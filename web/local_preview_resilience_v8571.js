// TEST v8.5.71 — resilient LOCAL preview with silent-hang watchdog.
// Images still use the fast direct <img> path first. If WebView never emits
// load/error, a watchdog promotes the same local file to a Blob fallback.
// The fallback identifies the real image format from magic bytes before using
// extension/header hints. No HDD rescan, no MEGA traffic, no JD changes.
(() => {
  'use strict';
  ...
  async function blobRetry(img, reason = 'error') {
    ...
    setLocalType(reason === 'watchdog' ? 'IMAGE • FALLBACK' : 'IMAGE • RETRY', reason === 'watchdog' ? 'Preview-ul direct nu a răspuns la timp; încerc fallback local.' : 'Reîncarc imaginea locală prin fallback Blob.');
    ...
    try {
      ...
      // If the direct request finally decoded while fallback I/O was running,
      // keep the fast direct result and abort promotion to Blob.
      if (img.dataset.ddgLocalStage === 'direct-late' && img.naturalWidth > 0) return;
      ...
      setLocalType('IMAGE • DECODARE', `${detected.mime} detectat prin ${detected.source}; aștept decoderul WebView.`);
    } catch (error) {
      if (error?.name === 'AbortError' && img.naturalWidth > 0) return;
      ...
      renderError(img, 'Imagine locală indisponibilă', `DDG nu a putut finaliza preview-ul local: ${message}`);
    }
  }
  ...
  function onImageLoad(img) {
    ...
    let stage = String(img.dataset.ddgLocalStage || 'direct');
    if (stage === 'blob-fetch' && !String(img.src || '').startsWith('blob:')) {
      cancelFallback(img);
      img.dataset.ddgLocalStage = 'direct-late';
      stage = 'direct-late';
    }

    if (stage === 'blob') {
      const mime = String(img.dataset.ddgDetectedMime || 'image').trim();
      setLocalType('IMAGE • FALLBACK OK', `Preview local încărcat prin Blob • ${mime}.`);
    } else if (stage === 'direct-late') {
      setLocalType('IMAGE • DIRECT OK', 'Preview-ul direct a răspuns după pornirea fallback-ului.');
    } else {
      setLocalType('IMAGE', 'Preview local direct.');
    }
  ...
  function onImageError(img) {
    ...
    renderError(img, 'Imagine locală nerecunoscută de WebView', mime ? `Fișierul a fost citit corect și identificat ca ${mime}, dar decoderul WebView nu îl poate afișa.` : 'Fișierul există, dar WebView nu îl poate decoda. Deschide copia în aplicația externă.');
  }
  ...
  function armWatchdog(img) {
    ...
    const timer = setTimeout(() => {
      watchdogs.delete(img);
      if (!img.isConnected || String(img.dataset.ddgLocalStage || '') !== 'direct') return;
  ...
  function robustLocalPreviewHTML(path) {
    ...
    if (kind === 'image') {
      ...
      return `<img id="localImage" data-ddg-generation="${generation}" data-ddg-local-path="${safePath}" data-ddg-local-stage="direct" src="${url}" alt="Preview local" style="display:block;width:auto;height:auto;max-width:100%;max-height:420px;object-fit:contain" onload="ddgLocalPreviewV8571.onImageLoad(this)" onerror="ddgLocalPreviewV8571.onImageError(this)"><span class="miniInfo">${esc(ext)}</span>`;
    }
    if (kind === 'video') {
