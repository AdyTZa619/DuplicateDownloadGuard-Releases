// TEST v8.5.66 — deterministic JDownloader Click'n'Load handoff.
...
(() => {
  ...
  function buildSubmission(rows) {
    ...
    for (const row of rows || []) {
      ...
      seen.add(link);
      urls.push(link);
      descriptions.push(String(row?.remote?.name || row?.remote?.path || 'DDG'));
    }
    ...
    // Official JDExternInterface parameter is singular: description.
  ...
  async function checkJD() {
    return await new Promise(resolve => {
      document.getElementById('ddgJDCheckV8566')?.remove();
      window.jdownloader = false;
      const script = document.createElement('script');
      script.id = 'ddgJDCheckV8566';
  ...
  function submitOneForm(params) {
    // Never fetch() then retry: CORS can reject the response after JD accepted
    // the first POST, causing duplicate links. Submit the official form once.
  ...
  function installDialog() {
    if (document.getElementById('ddgJDFastDecisionV8566')) return;
    const style = document.createElement('style');
    style.id = 'ddgJDFastDecisionV8566Style';
    style.textContent = `
      #ddgJDFastDecisionV8566{position:fixed;inset:0;z-index:12050;background:rgba(3,7,12,.80);display:flex;align-items:center;justify-content:center;padding:24px}
      #ddgJDFastDecisionV8566.hidden{display:none}
      #ddgJDFastDecisionV8566 .box{width:min(720px,94vw);background:#0e1721;border:1px solid #32465a;border-radius:14px;box-shadow:0 24px 70px rgba(0,0,0,.45);overflow:hidden}
      #ddgJDFastDecisionV8566 .head{padding:16px 18px;border-bottom:1px solid #26394b;font-size:17px;font-weight:800}
      #ddgJDFastDecisionV8566 .body{padding:16px 18px;color:#c7d7e7;line-height:1.5}
      #ddgJDFastDecisionV8566 .summary{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:13px 0}
      #ddgJDFastDecisionV8566 .card{border:1px solid #2c4053;border-radius:10px;padding:11px;background:#0a121a}
      #ddgJDFastDecisionV8566 .card b{display:block;font-size:20px;margin-bottom:2px}
      #ddgJDFastDecisionV8566 .note{padding:10px 12px;border-left:3px solid #65b7ff;background:#102030;border-radius:8px;font-size:12px}
      #ddgJDFastDecisionV8566 .foot{display:flex;gap:9px;justify-content:flex-end;flex-wrap:wrap;padding:13px 18px;border-top:1px solid #26394b}
      @media(max-width:620px){#ddgJDFastDecisionV8566 .summary{grid-template-columns:1fr}}
    `;
    document.head.appendChild(style);
    document.body.insertAdjacentHTML('beforeend', `
      <div id="ddgJDFastDecisionV8566" class="hidden" role="dialog" aria-modal="true">
        <div class="box">
          <div class="head">JDownloader — fără rescanare HDD</div>
          <div class="body">
            <div id="ddgJDFastTextV8566"></div>
            <div class="summary">
              <div class="card"><b id="ddgJDFastDownloadV8566">0</b>Recomandate</div>
              <div class="card"><b id="ddgJDFastDuplicateV8566">0</b>Ai deja</div>
              <div class="card"><b id="ddgJDFastReviewV8566">0</b>De verificat</div>
            </div>
          ...
          <div class="foot">
            <button class="btn" type="button" id="ddgJDFastCancelV8566">Anulează</button>
            <button class="btn" type="button" id="ddgJDFastRecommendedV8566">Trimite recomandate</button>
            <button class="btn primary" type="button" id="ddgJDFastAllV8566">Trimite TOATE</button>
          </div>
        </div>
      </div>`);
    document.getElementById('ddgJDFastCancelV8566')?.addEventListener('click', closeDialog);
    document.getElementById('ddgJDFastRecommendedV8566')?.addEventListener('click', async () => {
      if (pending?.recommended?.length) await sendRowsNow(pending.recommended);
    });
    document.getElementById('ddgJDFastAllV8566')?.addEventListener('click', async () => {
      if (pending?.all?.length) await sendRowsNow(pending.all);
    });
  }

  function closeDialog() { document.getElementById('ddgJDFastDecisionV8566')?.classList.add('hidden'); }

  function showDialog(rows) {
    installDialog();
    const groups = splitRows(rows);
    pending = {all:rows.slice(), recommended:groups.DOWNLOAD.slice(), groups};
    document.getElementById('ddgJDFastTextV8566').innerHTML = `<b>${rows.length} fișier(e) selectate.</b> Alege exact ce trimiți în JDownloader.`;
    document.getElementById('ddgJDFastDownloadV8566').textContent = String(groups.DOWNLOAD.length);
    document.getElementById('ddgJDFastDuplicateV8566').textContent = String(groups.DUPLICATE.length);
    document.getElementById('ddgJDFastReviewV8566').textContent = String(groups.REVIEW.length);
    const recommended = document.getElementById('ddgJDFastRecommendedV8566');
    const all = document.getElementById('ddgJDFastAllV8566');
    ...
    document.getElementById('ddgJDFastDecisionV8566')?.classList.remove('hidden');
  }
  ...
  function install() {
    ...
    if (select && !select.dataset.ddgFastJDBoundV8566) {
      select.dataset.ddgFastJDBoundV8566 = '1';
      select.addEventListener('change', () => setTimeout(install, 0));
    }
    window.sendSelectedJD2 = sendBatchAware;
  }

  // Loaded before legacy JD handlers: this capture listener wins and stops them.
  ...
  window.ddgJDownloaderFastV8566 = {sendBatchAware, sendExactIDs, rowsForIDs, onePackageName, buildSubmission};
})();
