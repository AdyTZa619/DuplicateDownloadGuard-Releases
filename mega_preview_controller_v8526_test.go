...
func TestMegaPreviewTwentyWarmSwitchesRunNoMegaCommandsV8526(t *testing.T) {
	...
	defer server.Close()
	a := &App{preview: MegaPreviewState{Active: true, SourceURL: "mega-source", RemotePath: megaWarmRootRefV86, StreamURL: server.URL + "/root", Exe: "MegaClient.exe"}}
	...
	if calls.Load() != 0 {
		t.Fatalf("warm root executed %d MEGAcmd commands", calls.Load())
	}
	if last != 20 {
	...
	if trace.Route != "warm-root" || trace.Points["T4"].AtMS == 0 {
		t.Fatalf("last warm trace invalid: %#v", trace)
	}
}

func TestMegaPreviewABCShareOneSourceInitializationV8526(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Accept-Ranges", "bytes")
	...
	c := testPreviewControllerV8526(a, func(ctx context.Context, _ time.Duration, _ string, args ...string) (string, error) {
		...
		case <-ctx.Done():
			return "", ctx.Err()
		case <-release:
			return "Serving via webdav /: " + server.URL + "/root", nil
		}
	})
	...
	if !strings.HasSuffix(target, "/root/C.mp4") {
		t.Fatalf("last click did not own target: %s", target)
	}
	if calls.Load() != 1 {
		t.Fatalf("A/B/C started %d MEGAcmd commands, want one", calls.Load())
	}
...
func TestMegaPreviewRange206AndFirstByteTimingV8526(t *testing.T) {
	...
	defer server.Close()
	a := &App{preview: MegaPreviewState{Active: true, SourceURL: "range-source", RemotePath: megaWarmRootRefV86, StreamURL: server.URL + "/root"}}
	c := testPreviewControllerV8526(a, nil)
...
func TestMegaPreviewObsoleteGenerationReturnsGoneV8526(t *testing.T) {
	...
	defer server.Close()
	a := &App{preview: MegaPreviewState{Active: true, SourceURL: "source", RemotePath: megaWarmRootRefV86, StreamURL: server.URL + "/root"}}
	c := testPreviewControllerV8526(a, nil)
...
func TestMegaPreviewTwentyStreamingSwitchesDoNotLeakRequestsV8526(t *testing.T) {
	...
	defer server.Close()
	a := &App{preview: MegaPreviewState{Active: true, SourceURL: "stream-source", RemotePath: megaWarmRootRefV86, StreamURL: server.URL + "/root"}}
	c := testPreviewControllerV8526(a, nil)
...
func TestMegaPreviewUIOwnsT0ToT12AndNoAutomaticFallbackV8526(t *testing.T) {
	...
	if !strings.Contains(string(index), `<script defer src="/mega_preview_v8526.js"></script>`) {
		t.Fatal("v8.5.26 preview UI is not embedded")
	}
}
