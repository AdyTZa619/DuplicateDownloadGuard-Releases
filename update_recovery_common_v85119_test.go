...
func TestRepairInterruptedExecutableV85119RestoresReplacing(t *testing.T) {
	...
	if err := os.WriteFile(replacing, []byte("old-good"), 0755); err != nil { t.Fatal(err) }
	if err := os.WriteFile(copying, []byte("partial"), 0755); err != nil { t.Fatal(err) }
	if err := repairInterruptedExecutableV85119(current); err != nil { t.Fatal(err) }
	b, err := os.ReadFile(current)
	if err != nil || string(b) != "old-good" { t.Fatalf("current not restored: %q %v", string(b), err) }
	if _, err := os.Stat(copying); !os.IsNotExist(err) { t.Fatalf("stale .copying survived: %v", err) }
}

func TestRepairInterruptedExecutableV85119CleansTempsWhenCurrentExists(t *testing.T) {
	dir := t.TempDir()
	current := filepath.Join(dir, "DuplicateDownloadGuard_PRO.exe")
	if err := os.WriteFile(current, []byte("live"), 0755); err != nil { t.Fatal(err) }
	...
	for _, suffix := range []string{".copying", ".replacing"} {
		if _, err := os.Stat(current+suffix); !os.IsNotExist(err) { t.Fatalf("stale %s survived: %v", suffix, err) }
	}
}
