...
func TestLocalPreviewV8576LoadsBeforeProviderCompare(t *testing.T) {
	...
	local := strings.Index(s, "/local_preview_direct_safe_v8576.js")
	provider := strings.Index(s, "/provider_compare_ui_v8558.js")
	if core < 0 || local < 0 || provider < 0 {
		t.Fatal("expected preview core, direct-safe local preview v8576 and provider compare modules")
	}
